...
class InfiniteGridMenu {
  ...
  run(time = 0) {
    ...
    if (fpsUpdated) {
      // Performance monitoring updates - images don't need constant updates like videos
    }
  ...
  #initTexture() {
    ...
    // Store images for optimized updating
    this.images = [];
    this.imageNeedsUpdate = new Array(itemCount).fill(true);
    ...
    this.imageCanvas.width = this.atlasSize * this.cellSize;
    this.imageCanvas.height = this.atlasSize * this.cellSize;

    // Balanced rendering quality settings
    this.imageCtx.imageSmoothingEnabled = true;
    this.imageCtx.imageSmoothingQuality = isHighPerformanceDevice ? 'high' : 'medium';
    ...
    this.resourceManager.register('imageCanvas', this.imageCanvas, () => {
      if (this.imageCanvas) {
        this.imageCanvas.width = 1;
        this.imageCanvas.height = 1;
      }
    });

    console.log('Creating image texture with items:', this.items);

    Promise.all(
      this.items.map(
        (item, index) =>
          new Promise((resolve) => {
            console.log(`Loading image ${index}:`, item.cover);

            const img = new Image();
            img.crossOrigin = 'anonymous';

            img.addEventListener('load', () => {
              console.log(`Image loaded for item ${index}`);
              clearTimeout(timeoutId);
              resolve(img);
            });

            img.addEventListener('error', (e) => {
              console.warn(`Image loading error for item ${index}:`, e);
              ...
              fallbackCanvas._isImageFallback = true;
              resolve(fallbackCanvas);
            });

            // Add timeout fallback to prevent hanging
            const timeoutId = setTimeout(() => {
              console.warn(`Image loading timeout for item ${index}`);
              const fallbackCanvas = document.createElement('canvas');
              fallbackCanvas.width = 512;
              fallbackCanvas.height = 512;
              const fallbackCtx = fallbackCanvas.getContext('2d');
      ...
      this.images = images;
      this.#updateImageTexture(true); // Force initial update
    }).catch(error => {
      console.error('Error loading image textures:', error);
    });
  }

  #updateImageTexture(forceUpdate = false) {
    if (!this.images || this.images.length === 0) return;
    
    // Images only need to be updated once (unlike videos)
    if (!forceUpdate && !this.textureNeedsUpdate) return;
    ...
    this.images.forEach((image, i) => {
      ...
      // Check if this is an image element or canvas fallback
      const isImage = image && image.tagName === 'IMG';
      const isCanvas = image && image.tagName === 'CANVAS';
      
      // Images only need to be drawn once (unlike videos which need constant updates)
      const needsUpdate = forceUpdate || this.textureNeedsUpdate;
      
      if (!needsUpdate && !isCanvas) return;

      try {
        if (isImage && image.width && image.height) {
          this.imageCtx.clearRect(x, y, cellSize, cellSize);
          this.imageCtx.drawImage(image, x, y, cellSize, cellSize);
          textureUpdated = true;
        } else if (isCanvas && image.width && image.height) {
          // Fallback canvas
          this.imageCtx.clearRect(x, y, cellSize, cellSize);
          this.imageCtx.drawImage(image, x, y, cellSize, cellSize);
          textureUpdated = true;
          ...
          this.imageCtx.fillStyle = '#608B83';
          this.imageCtx.fillRect(x, y, cellSize, cellSize);
          textureUpdated = true;
        }
      } catch {
    ...
    if (textureUpdated) {
      ...
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, this.imageCanvas);
  ...
  #animate(deltaTime) {
    ...
    // Update image textures with throttling
  ...
  dispose() {
    ...
    if (this.images) {
      this.images.forEach(image => {
        // Check if this is actually an image element (not a canvas fallback)
  ...
  setVisibility(visible) {
    ...
    // Images are static and don't need pause/play management like videos
  }
}
...
export default function InfiniteMenu() {
  ...
  // Ensure hydration is complete before running client-side code
  useEffect(() => {
    setIsHydrated(true);
  }, []);
